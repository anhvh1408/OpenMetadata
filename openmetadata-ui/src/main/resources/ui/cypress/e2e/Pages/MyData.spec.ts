...
const entities = {
  ...
  topic: {
    request: {
      url: '/audax/openmetadata/api/v1/topics',
  ...
  dashboard: {
    request: {
      url: '/audax/openmetadata/api/v1/dashboards',
  ...
  pipeline: {
    request: {
      url: '/audax/openmetadata/api/v1/pipelines',
  ...
  mlmodel: {
    request: {
      url: '/audax/openmetadata/api/v1/mlmodels',
  ...
  container: {
    request: {
      url: '/audax/openmetadata/api/v1/containers',
...
const updateOwnerAndVerify = ({ url, body, type, entityName, newOwner }) => {
  interceptURL(
    'GET',
    '/audax/openmetadata/api/v1/users/loggedInUser?*',
    'loggedInUser'
  );
  interceptURL(
    'GET',
    '/audax/openmetadata/api/v1/feed?type=Conversation&filterType=OWNER_OR_FOLLOWS&userId=*',
    'feedData'
  );
...
const prepareData = () => {
  ...
  cy.getAllLocalStorage().then((data) => {
    ...
    cy.request({
      ...
      cy.request({
        method: 'GET',
        url: `/audax/openmetadata/api/v1/teams/name/Organization`,
        headers: { Authorization: `Bearer ${token}` },
      }).then((teamResponse) => {
        cy.request({
          method: 'POST',
          url: `/audax/openmetadata/api/v1/teams`,
          headers: { Authorization: `Bearer ${token}` },
          body: {
            name: team.name,
      ...
      tables.forEach((table) => {
        cy.request({
          ...
          cy.request({
            method: 'PUT',
            url: `/audax/openmetadata/api/v1/tables/${tableResponse.body.id}/followers`,
            headers: {
              Authorization: `Bearer ${token}`,
...
const cleanUp = () => {
  ...
  cy.getAllLocalStorage().then((data) => {
    ...
    [user1Id, user2Id].forEach((id) => {
      cy.request({
        method: 'DELETE',
        url: `/audax/openmetadata/api/v1/users/${id}?hardDelete=true&recursive=false`,
        headers: { Authorization: `Bearer ${token}` },
      });
    });
    cy.request({
      method: 'DELETE',
      url: `/audax/openmetadata/api/v1/teams/${team.id}?hardDelete=true&recursive=true`,
      headers: { Authorization: `Bearer ${token}` },
    });
  });
};
...
describe('My Data page', { tags: 'DataAssets' }, () => {
  ...
  it('Verify my data widget', () => {
    ...
    verifyEntities({
      url: '/audax/openmetadata/api/v1/search/query?q=*&index=all&from=0&size=25',
    });
  ...
  it('Verify following widget', () => {
    ...
    verifyEntities({
      url: '/audax/openmetadata/api/v1/search/query?q=*followers:*&index=all&from=0&size=25',
    });
    cy.logout();
  });
