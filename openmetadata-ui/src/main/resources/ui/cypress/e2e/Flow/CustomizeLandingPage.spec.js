...
describe('Customize Landing Page Flow', { tags: 'Settings' }, () => {
  ...
  before(() => {
    ...
    cy.getAllLocalStorage().then((data) => {
      ...
      cy.request({
        method: 'GET',
        url: `/audax/openmetadata/api/v1/users/loggedInUser`,
        headers: { Authorization: `Bearer ${token}` },
      }).then((userResponse) => {
        // Create a persona
        cy.request({
          method: 'POST',
          url: `/audax/openmetadata/api/v1/personas`,
          headers: { Authorization: `Bearer ${token}` },
          ...
          cy.request({
            method: 'PATCH',
            url: `/audax/openmetadata/api/v1/users/${testData.user.id}`,
            headers: {
  ...
  after(() => {
    ...
    cy.request({
      method: 'DELETE',
      url: `/audax/openmetadata/api/v1/personas/${testData.persona.id}`,
      headers: { Authorization: `Bearer ${token}` },
    });
    ...
    cy.request({
      method: 'DELETE',
      url: `/audax/openmetadata/api/v1/docStore/${testData.docStoreData.id}`,
      headers: { Authorization: `Bearer ${token}` },
    });
  });
  ...
  it('Creation of custom landing page config and widget removal should work properly', () => {
    ...
    // Save layout
    interceptURL('POST', `/audax/openmetadata/api/v1/docStore`, 'getMyData');
