...
const userURL =
  '/audax/openmetadata/api/v1/search/query?q=**%20AND%20isBot:false&from=0&size=0&index=user_search_index';
const teamURL =
  '/audax/openmetadata/api/v1/search/query?q=*%20AND%20teamType:Group&from=0&size=10&index=team_search_index&sort_field=displayName.keyword&sort_order=asc';
...
export const addOwner = (ownerName: string, dataTestId?: string) => {
  ...
  cy.log('/audax/openmetadata/api/v1/users?limit=*&isBot=false*');
  ...
  interceptURL('PATCH', `/audax/openmetadata/api/v1/**`, 'patchOwner');
...
export const updateOwner = (ownerName: string, dataTestId?: string) => {
  ...
  interceptURL(
    'GET',
    `audax/openmetadata/api/v1/search/query?q=*${encodeURI(
      ownerName
...
export const addTeamAsOwner = (teamName: string, dataTestId?: string) => {
  interceptURL(
    'GET',
    '/audax/openmetadata/api/v1/search/query?q=*&from=0&size=*&index=team_search_index&sort_field=displayName.keyword&sort_order=asc',
    'getTeams'
  );
  ...
  interceptURL(
    'GET',
    `audax/openmetadata/api/v1/search/query?q=*${encodeURI(teamName)}*`,
    'searchTeams'
  );
